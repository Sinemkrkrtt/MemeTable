...
export default function JokerModal({ visible, onClose, onUseJoker, selectedCard }) {
  ...
  // 🚀 GERÇEK ZAMANLI BAKIYE VE ENVANTER
  ...
  const handleBuyJoker = async (joker) => {
    ...
    if (userCoins >= joker.price) {
      try {
        ...
        // İstersen buraya bir "Satın Alındı" ses efekti ekleyebilirsin
      } catch (error) {
          ...
          <View style={styles.grid}>
           {JOKERS.map((joker, index) => {
              ...
              if (!hasItem) {
                ...
                onBtnPress = () => {
                  onUseJoker(joker.id);
                  onClose(); // Kullanınca modalı kapat
                };
              }
...
const styles = StyleSheet.create({
  absoluteOverlay: { ...StyleSheet.absoluteFillObject, zIndex: 9999, elevation: 9999, justifyContent: 'center', alignItems: 'center' },
  overlayBackground: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(15, 23, 42, 0.7)' },
  modalContent: {
  ...
  sidebarTopIcon: { width: 45, height: 45, borderRadius: 16, backgroundColor: '#FFF', justifyContent: 'center', alignItems: 'center', marginBottom: 30, elevation: 5, shadowColor: palet.pink, shadowOpacity: 0.3, shadowRadius: 8, shadowOffset: { width: 0, height: 4 } },
  textContainer: { flex: 1, justifyContent: 'center' },
  sidebarTitle: { fontFamily: 'Nunito_900Black', fontSize: 13, color: palet.pink, transform: [{ rotate: '-90deg' }], width: 150, textAlign: 'center', letterSpacing: 4 },
  coinBadge: { backgroundColor: '#2D2D2D98', paddingHorizontal: 10, paddingVertical: 8, borderRadius: 12, alignItems: 'center', marginTop: 20, shadowColor: '#000', shadowOpacity: 0.2, shadowRadius: 5 },
  coinText: { fontFamily: 'Nunito_800ExtraBold', color: 'white', fontSize: 10, marginTop: 4 },
  mainContent: { flex: 1, padding: 25, backgroundColor: palet.bg },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 },
  headerLeft: { flexDirection: 'row', alignItems: 'center' },
  headerTitle: { fontFamily: 'Nunito_900Black', fontSize: 22, color: palet.textDark, letterSpacing: -0.5 },
  activeBadge: { marginLeft: 12, backgroundColor: '#FEE0FD', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 10, flexDirection: 'row', alignItems: 'center' },
  dot: { width: 6, height: 6, borderRadius: 3, backgroundColor: '#FF2AD1', marginRight: 5 },
  activeText: { fontFamily: 'Nunito_900Black', fontSize: 10, color: '#FF2AD1', letterSpacing: 1 },
  closeCircle: { width: 36, height: 36, borderRadius: 18, backgroundColor: 'white', justifyContent: 'center', alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 5, elevation: 2 },
  grid: { flex: 1, flexDirection: 'row', justifyContent: 'space-between', gap: 12 },
  card: { width: '31%', height: '100%', borderRadius: 28, padding: 14, alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#FFF', borderWidth: 1, borderColor: 'rgba(0,0,0,0.02)', shadowColor: '#94A3B8', shadowOpacity: 0.15, shadowRadius: 15, shadowOffset: { width: 0, height: 8 }, elevation: 4 },
  imgWrapper: { width: 65, height: 65, borderRadius: 22, backgroundColor: '#F8FAFC', justifyContent: 'center', alignItems: 'center' },
  jokerImg: { width: 42, height: 42 },
  badge: { position: 'absolute', top: -6, right: -6, minWidth: 22, height: 22, borderRadius: 11, justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: '#FFF', shadowColor: '#000', shadowOpacity: 0.2, shadowRadius: 3 },
  badgeText: { fontFamily: 'Nunito_900Black', color: '#FFF', fontSize: 10 },
  cardInfo: { alignItems: 'center', marginVertical: 8 },
  jokerName: { fontFamily: 'Nunito_900Black', fontSize: 13, textAlign: 'center', color: palet.textDark, letterSpacing: -0.2 },
  jokerDesc: { fontFamily: 'Nunito_600SemiBold', fontSize: 10, textAlign: 'center', color: '#64748B', marginTop: 4, lineHeight: 14 },
  useBtn: { width: '100%', height: 42, borderRadius: 14, overflow: 'hidden', shadowColor: palet.pink, shadowOpacity: 0.2, shadowRadius: 5, shadowOffset: { width: 0, height: 3 } },
  btnGradient: { flex: 1, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' },
  btnText: { fontFamily: 'Nunito_900Black', color: '#FFF', fontSize: 12, letterSpacing: 0.5 },
});