...
export default function AuthScreen({ route, navigation }) {
  ...
  const handleGuestLogin = async () => {
    setLoading(true);
    try {
      const hasUsedGuest = await AsyncStorage.getItem('hasUsedGuestAccount');
      
      if (hasUsedGuest === 'true') {
        showAlert("Misafir Limiti Doldu", "Bu cihazda misafir hakkınızı zaten kullandınız. Oynamaya devam etmek için lütfen kayıt olun!");
        setLoading(false);
        return; 
      }
      ...
      const guestNickname = "Misafir_" + user.uid.substring(0, 4).toUpperCase();
      await updateProfile(user, { displayName: guestNickname });
      ...
      await AsyncStorage.setItem('hasUsedGuestAccount', 'true');

      navigation.replace('Home', { isNewGuest: true });

    } catch (error) {
      console.log("Misafir Girişi Hatası:", error);
      showAlert("Hata", "Misafir girişi yapılamadı. Bağlantınızı kontrol edin.");
    } finally {
      setLoading(false);
    }
  };
