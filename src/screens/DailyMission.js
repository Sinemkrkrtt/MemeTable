...
export default function DailyMission({ wonHearts, onRefreshUser }) {
  ...
  // 🚀 Başlangıç Ayarları
  useEffect(() => {
    setAudioModeAsync({ playsInSilentMode: true }).catch(e => console.log(e));
  }, []);
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          if (data.lastMissionDate !== today) {
            // Eğer tarih bugünden farklıysa (yeni gün), her şeyi sıfırla
        ...
        console.log("Görev kontrol hatası:", error.code);
      }
    };
    
    checkDailyReset();
  }, [wonHearts]); // wonHearts değiştikçe kontrol et
  ...
  const handleInstantOpen = () => {
    if (localHearts < 3 || isBoxOpened || isClaiming) return;
  ...
  const handleClaimReward = async () => {
    ...
    if (user) {
      ...
      try {
        ...
        // Ödül tipine göre increment işlemleri
        if (currentReward.type === 'joker') {
          const fieldMap = { 'joker1': 'joker_skip', 'joker2': 'joker_double', 'joker3': 'joker_freeze' };
          updateData[fieldMap[currentReward.id]] = increment(1);
          ...
          <View>
            ...
            <Text style={styles.missionSubTitle}>
              {isBoxOpened 
                ...
                : localHearts < 3 
                  ? `Paketi açmak için ${3 - localHearts} galibiyet lazım` 
                  : "Hediye paketin hazır!"}
            </Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.questMainRow}>
          ...
          <View style={styles.questContent}>
            <Text style={[styles.questText, isBoxOpened && { color: palet.gray }]}>
              {isBoxOpened ? "Bugünlük Bu Kadar" : "3 Maç Kazan, Hediyeni Kap!"}
            </Text>
            <View style={styles.progressRow}>
              <View style={styles.progressTrack}>
                <LinearGradient 
                    colors={isBoxOpened ? [palet.gray, palet.gray] : [palet.vibrant, palet.orange]} 
                    style={[styles.progressFill, { width: isBoxOpened ? '100%' : `${(Math.min(localHearts, 3) / 3) * 100}%` }]} 
                />
              </View>
          ...
          <View style={styles.giftWrapper}>
            {isBoxOpened ? (
              <View style={styles.lockedGift}>
                <Ionicons name="checkmark-circle" size={32} color='#FF8E1E' />
                <Text style={[styles.lockedText, { color: '#FF8E1E' }]}>ALINDI</Text>
              </View>
            ) : localHearts < 3 ? (
              <View style={styles.lockedGift}>
          ...
          <View style={styles.lootMainStage}>
            <Animated.View style={{ 
              ...
              <View style={styles.premiumObjectFrame}>
                <View style={styles.objectInner}>
                  <LinearGradient colors={['rgba(255,255,255,0.9)', 'transparent']} style={styles.shineOverlay} />
              ...
              <TouchableOpacity style={styles.claimButton} onPress={handleClaimReward} disabled={isClaiming}>
                <LinearGradient colors={['#FF69EB', '#FF00D6']} style={styles.claimGradient}>
                  <Text style={styles.claimText}>{isClaiming ? 'İŞLENİYOR...' : 'HEDİYENİ AL'}</Text>
                </LinearGradient>
              </TouchableOpacity>
